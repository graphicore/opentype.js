var opentype = (() => {
  ...
  function fontToSfntTable(font) {
    ...
    const gaspTable = font.tables.gasp ? gasp_default.make(font.tables.gasp) : void 0;
    const languageTags = [];
  ...
  function parseWOFFTableEntries(data, numTables) {
    ...
    for (let i = 0; i < numTables; i += 1) {
      ...
      const checksum = parse_default.getULong(data, p + 16);
      let compression;
  ...
  function woffToOTF(buffer) {
    if (buffer.constructor !== ArrayBuffer)
      buffer = new Uint8Array(buffer).buffer;
    const data = new DataView(buffer, 0), out = [], signature = parse_default.getTag(data, 0);
    if (signature !== "wOFF")
      throw new Error(`TYPE ERROR signature must be wOFF but is: "${signature}"`);
    const flavor = parse_default.getTag(data, 4), numTables = parse_default.getUShort(data, 12), tableEntries = parseWOFFTableEntries(data, numTables), max = [];
    for (let n = 0; n < 64; n++) {
      if (Math.pow(2, n) > numTables)
        break;
      max.splice(0, Infinity, n, 2 ** n);
    }
    const searchRange2 = max[1] * 16, entrySelector = max[0], rangeShift = numTables * 16 - searchRange2;
    out.push(
      ...encode.TAG(flavor),
      ...encode.USHORT(numTables),
      ...encode.USHORT(searchRange2),
    ...
    const initialData = new Uint8Array(out.length), buffers = [initialData];
    for (let i = 0, l = out.length; i < l; i++)
      initialData[i] = out[i];
    for (let i = 0; i < numTables; i++) {
      const tableEntry = tableEntries[i], table = uncompressTable(data, tableEntry), offset2 = tableEntry.outOffset + tableEntry.length, padding = offset2 % 4 !== 0 ? 4 - offset2 % 4 : 0;
      buffers.push(
        new Uint8Array(table.data.buffer, table.offset, tableEntry.length),
        new Uint8Array(padding)
      );
    }
    const result = new Uint8Array(buffers.reduce((accum, buffer2) => accum + buffer2.byteLength, 0));
    buffers.reduce((offset2, buffer2) => {
      result.set(buffer2, offset2);
      return offset2 + buffer2.byteLength;
    }, 0);
